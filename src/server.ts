...
function formatTask(result: TaskResult) {
  ...
  const fileList = artifacts.files.length
    ? artifacts.files
        .map(
          (f) =>
            `- ${f.path} (${f.bytes} bytes${f.width ? `, ${f.width}x${f.height}` : ""})${
              f.inlined ? " [shown below]" : ""
            }${f.note ? ` - ${f.note}` : ""}`,
        )
        .join("\n")
    : "(none)";
    ...
    "codex_generate_image",
    {
      ...
      inputSchema: {
        ...
        size: z
          ...
          .optional()
          .describe(
            'Pixel size as WIDTHxHEIGHT, e.g. "1024x1024" (fastest, the default), "1536x1024" ' +
              'landscape, "1024x1536" portrait, or "auto". This is a request to Codex; the actual ' +
              "dimensions are reported in the result.",
          ),
        quality: z
